...
export function RecursiveGroupContainer({
  ...
  if (item.type === 'group') {
    ...
    if (item.completed && !item.active) {
      ...
      const completedFields = visibleChildren.filter(child =>
  ...
  if (item.type === 'field' && item.fieldType) {
    const PluginComponent = globalRegistry.getComponent(item.fieldType);
    ...
    const isLastInGroup = parent ? siblingIndex === parent.children.length - 1 : true;
    ...
    return (
      <Box marginLeft={baseIndent}>
        <PluginComponent
          key={`plugin-${item.id}-${item.depth}-${isActive ? 'active' : isCompleted ? 'completed' : 'pending'}`}
