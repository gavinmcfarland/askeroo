...
export const ui = {
  ...
  async showGroup(label: string | undefined, flow?: 'phase' | 'static', id?: string, staticFields?: Array<{ id: string; message: string; kind: string; opts: any }>): Promise<void> {
    appInstance.currentGroup = label;
    const promptFn = await ensureApp();
    await promptFn({ type: 'group', id: id || generatePromptId('group', label || 'group'), message: label, flow, staticFields });
  },
