...
console.log('\n💡 Fix verification:');
console.log('- Total nodes:', stats.totalNodes, '(should be 2: root + text)');
console.log('- Active node:', stats.activeNodeId, '(should be simple-text)');
console.log('- Visible children:', visibleChildren.length, '(should be 1)');

if (stats.totalNodes === 2 && stats.activeNodeId === 'simple-text' && visibleChildren.length === 1) {
