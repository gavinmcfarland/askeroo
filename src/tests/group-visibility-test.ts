...
function testVisibilityFiltering(groupNode: any, showOnlyActiveAndCompleted: boolean = false) {
  ...
  console.log(`  - showOnlyActiveAndCompleted: ${showOnlyActiveAndCompleted}`);
  ...
  const visibleChildren = groupNode.children.filter((child: any) => {
    ...
    if (showOnlyActiveAndCompleted) {
      ...
      console.log(`      - visible (showOnlyActiveAndCompleted): ${isVisible}`);
      return isVisible;
    }
    ...
    if (!groupNode.flow && groupNode.id === 'root') {
      ...
      console.log(`      - visible (root, first pending): ${allPreviousCompleted}`);
      return allPreviousCompleted;
    }

    // For progressive/phased groups, show completed, active, and the next pending field
    if (child.completed || child.active) {
      console.log(`      - visible (progressive, completed/active): true`);
      return true;
    }
    ...
    if (groupNode.flow === 'progressive') {
      ...
      const childIndex = siblings.indexOf(child);
      const allPreviousCompleted = siblings.slice(0, childIndex).every((prev: any) => prev.completed);
      console.log(`      - visible (progressive, next pending): ${allPreviousCompleted}`);
      return allPreviousCompleted;
    }
  ...
  if (visibleChildren.length === 0) {
    console.log(`  ❌ No visible children - group will not render (LABEL WON'T SHOW)`);
    return false;
  }
...
console.log('  2. All children are in pending state and visibility rules filter them out');
console.log('  3. Need to adjust visibility logic to show groups with labels even if no visible children');

console.log('\nStep 5: Recommended fix:');
console.log('🔧 Consider showing group labels even when no children are visible');
console.log('   - This would mean removing the "return null" when visibleChildren.length === 0');
