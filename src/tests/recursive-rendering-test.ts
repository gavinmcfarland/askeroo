...
function analyzeNode(node: any, indent = 0): void {
  ...
  const status = node.active ? '[ACTIVE]' : node.completed ? '[COMPLETED]' : node.visited ? '[VISITED]' : '[PENDING]';
...
const completedGroups = treeManager.findNodes(node =>
  node.type === 'group' && node.completed
);
console.log(`✓ Found ${completedGroups.length} completed groups`);

// Test field visibility
const visibleFields = treeManager.findNodes(node =>
...
const staticGroups = treeManager.findNodes(node =>
  node.type === 'group' && node.flow === 'static'
);
