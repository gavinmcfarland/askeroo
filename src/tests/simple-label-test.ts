...
if (!hasChildren) {
  console.log(`3. Empty group check: ${!groupNode.children || groupNode.children.length === 0}`);
...
if (groupNode.children && groupNode.children.length > 0) {
  ...
  console.log(`Visible children with showOnlyActiveAndCompleted=true: ${visibleWithFilter.length}`);

  if (visibleWithFilter.length === 0) {
    console.log('✅ This would trigger the visibleChildren.length === 0 condition');
    console.log('✅ With my fix, should show label even with no visible children');
  }
  ...
  const visibleNormally = child.completed || child.active || allPreviousCompleted;
