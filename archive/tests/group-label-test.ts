...
function inspectLabels(node: any, depth = 0) {
	...
	console.log(
		`${indent}  - label: "${node.label}" (type: ${typeof node.label})`
	);
...
const scenarios = [
	...
	{
		name: "Active group with children",
		active: true,
		completed: false,
		hasChildren: true,
	},
	...
	{
		name: "Pending group with children",
		active: false,
		completed: false,
		hasChildren: true,
	},
...
scenarios.forEach((scenario) => {
	...
	console.log(
		`  - Active: ${scenario.active}, Completed: ${scenario.completed}, HasChildren: ${scenario.hasChildren}`
	);

	// Based on RecursiveGroupContainer logic
	const willShowGroup =
		scenario.hasChildren &&
		(scenario.active ||
			scenario.completed ||
			(!scenario.active && !scenario.completed));
...
console.log(
	"  3. Ensure group is in active/completed state or has visible children"
);
